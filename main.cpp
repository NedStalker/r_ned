...
int main()
{
	...
	"R_Ned by NedStalker\n"
	"Author: NedStalker\n"
	"Source Code: https://github.com/CowNowK/R_Ned\n"
	"\n"
	"- Нажми [INS] чтобы спрятать(увидеть).\n"
	"\n"
	"#Предпреждение:\n"
	"- Пользуйся на свой страх и риск\n"
	"- Не рекомендую пользваться на основных аккаунтах"
	"\n"
	"#Соавторы: \n"
	"Anders, NedStalker\n"
	"\n"
	...
	case 1:
		...
		goto END;
	case 2: 
		std::cout << "[ERROR] Failed to hook process, please run the cheat as Administrator (Right click R_Ned > Run as Adminstrator)." << std::endl; 
		goto END; 
	case 3:
	...
	*/
	...
	catch (OSImGui::OSException& e)
	{
		try
		{
			...
			Gui.AttachAnotherWindow("·´¿־«Ӣ£ºȫÇò¹¥ÊÆ", "SDL_app", Cheats::Run);
		}
		catch (OSImGui::OSException& e)
		{
